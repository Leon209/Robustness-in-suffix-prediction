...
###NEW
def get_train_test_val_datasets(event_log_location, event_log_properties, suffix_len = 1):
    event_log = CSV2EventLog(event_log_location, **event_log_properties)
    splitter = EventLogSplitter(**event_log_properties)
    train_df, val_df, test_df = splitter.split(event_log)
    return train_df, val_df, test_df

def extract_feature_info(df, properties):
    """
    Extract all categories for categorical features and ranges for continuous features.
    Computes both global (across all activities) and local (per-activity) feature info.
    
    Args:
        df: DataFrame containing the event log data
        properties: Dictionary containing 'categorical_columns', 'continuous_columns', and 'concept_name' keys
    
    Returns:
        Dictionary with 'global' and 'local' keys:
        - 'global': {
            'categorical': dict mapping column name -> list of unique categories,
            'continuous': dict mapping column name -> {'min': float, 'max': float, 'mean': float, 'std': float}
          }
        - 'local': {
            activity_name: {
                'categorical': dict mapping column name -> list of unique categories,
    ...
    """
    # Global feature info (existing logic)
    global_categories_info = {}
    global_ranges_info = {}
    
    # Get all unique categories for each categorical column (global)
    categorical_columns = properties.get('categorical_columns', [])
    for col in categorical_columns:
        if col in df.columns:
            # Get all unique values (excluding NaN and EOS)
            unique_values = df[col].dropna().unique()
            # Filter out EOS values and convert to list and sort for consistency
            global_categories_info[col] = sorted([str(val) for val in unique_values if pd.notna(val) and str(val) != 'EOS'])
        else:
            global_categories_info[col] = []
    
    # Get min/max ranges for each continuous column (global)
    continuous_columns = properties.get('continuous_columns', []) + properties.get('continuous_positive_columns', [])
    for col in continuous_columns:
        if col in df.columns:
            # Get non-null values, excluding EOS (in case EOS appears in continuous columns)
            col_data = df[col].dropna()
            # Filter out EOS values if they exist
            # Handle both string and numeric types by converting to string for comparison
            mask = col_data.astype(str) != 'EOS'
            col_data = col_data[mask]
            if len(col_data) > 0:
                global_ranges_info[col] = {
                ...
                global_ranges_info[col] = {'min': 0.0, 'max': 0.0, 'mean': 0.0, 'std': 0.0}
        else:
            global_ranges_info[col] = {'min': 0.0, 'max': 0.0, 'mean': 0.0, 'std': 0.0}
    
    # Local feature info (per-activity)
    local_info = {}
    activity_col = properties.get('concept_name', 'Activity')
    
    if activity_col in df.columns:
        # Group by activity and compute feature info for each activity
        for activity_name, activity_df in df.groupby(activity_col):
            # Skip EOS events
            if activity_name == 'EOS':
                continue
            
            activity_categories_info = {}
            activity_ranges_info = {}
            
            # Get categorical feature info for this activity
            for col in categorical_columns:
                if col in activity_df.columns:
                    unique_values = activity_df[col].dropna().unique()
                    # Filter out EOS values
                    filtered_values = [str(val) for val in unique_values if pd.notna(val) and str(val) != 'EOS']
                    if len(filtered_values) > 0:
                        activity_categories_info[col] = sorted(filtered_values)
                    # If no data for this feature in this activity, skip it (empty dict entry)
            
            # Get continuous feature info for this activity
            for col in continuous_columns:
                if col in activity_df.columns:
                    col_data = activity_df[col].dropna()
                    # Filter out EOS values if they exist
                    # Handle both string and numeric types by converting to string for comparison
                    mask = col_data.astype(str) != 'EOS'
                    col_data = col_data[mask]
                    if len(col_data) > 0:
                        activity_ranges_info[col] = {
                            'min': float(col_data.min()),
                            'max': float(col_data.max()),
                            'mean': float(col_data.mean()),
                            'std': float(col_data.std())
            ...
            # Only add activity if it has at least some feature info
            if activity_categories_info or activity_ranges_info:
                local_info[activity_name] = {
                    'categorical': activity_categories_info,
                    'continuous': activity_ranges_info
                }
    
    return {
        'global': {
            'categorical': global_categories_info,
            'continuous': global_ranges_info
        },
        'local': local_info
    }
###NEW
